...
def eqr_flops(cfg, seq_len):
    # EqR-style local baseline shares one recurrent level for low and high updates.
    level = cfg.h_cycles * (cfg.l_cycles + 1) * cfg.l_layers * token_block_flops(seq_len, cfg.d_model, cfg.dim_ff_mult)
    heads = seq_len * cfg.d_model * (cfg.vocab_size + cfg.num_classes)
    return level + heads
...
def lg_prm_flops(cfg, seq_len):
    ...
    pi_out = d * d
    step = explorer + pi_in + pi_layers + pi_out
...
def estimate_flops(cfg, seq_len):
    ...
    if cfg.model_type == "eqr":
        return eqr_flops(cfg, seq_len)
    if cfg.model_type == "lg_prm":
        return lg_prm_flops(cfg, seq_len)
